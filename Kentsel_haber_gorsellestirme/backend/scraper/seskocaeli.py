...
class SesKocaeliScraper(BaseScraper):
    ...
    def get_news(self) -> list[dict]:
        ...
        for a in articles:
            ...
            # SES Kocaeli haber URL'leri /haber/ içerir
    ...
    def _parse_article(self, url: str) -> dict | None:
        ...
        try:
            # Başlık — SES Kocaeli genellikle .haber-baslik veya .haber_baslik kullanır
            title_tag = (
                soup.find("h1", class_=re.compile("baslik|title|entry", re.I)) or
                soup.find("h1")
            )
            if not title_tag:
                return None
            ...
            # İçerik — .haber-icerik veya .haber-detay
            content_tag = (
                soup.find("div", class_=re.compile("icerik|detay|content|article|haber", re.I)) or
                soup.find("article")
            )
